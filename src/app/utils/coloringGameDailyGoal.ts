...
const STORAGE_KEY = 'coloringGame_dailyGoal';

// 현재 메모리상의 누적 점수를 추적하는 변수 (로컬에 저장 안 함)
let tempAccumulatedScore = 0;
...
export const resetDailyGoal = (): void => {
  ...
  tempAccumulatedScore = 0;
};

// 게임 시작 시 호출: 로컬 저장소의 점수를 메모리로 로드
export const loadAccumulatedScore = (): number => {
  const data = getDailyGoalData();
  tempAccumulatedScore = data.accumulatedScore;
  return tempAccumulatedScore;
};
...
// 그림 완성 시 호출: 메모리에만 점수 추가 (로컬 저장 X)
export const addScoreToMemory = (score: number): void => {
  tempAccumulatedScore += score;
};

// 게임 종료 또는 목표 달성 시 호출: 메모리 → 로컬 저장소에 저장
export const commitScore = (): boolean => {
  const data = getDailyGoalData();
  data.accumulatedScore = tempAccumulatedScore;
  ...
  if (data.targetScore !== null && data.targetScore > 0 && data.accumulatedScore >= data.targetScore) {
    data.achieved = true;
    saveDailyGoalData(data);  // 목표 달성 시 저장
    return true;  // 목표 달성!
  }
  
  // 일반 게임 종료 (하트 0)
  saveDailyGoalData(data);  // 게임 종료 시 저장
  return false;
};

// 일시정지에서 나가기: 메모리 점수를 되돌림 (로컬에는 영향 X)
export const revertScore = (score: number): void => {
  tempAccumulatedScore = Math.max(0, tempAccumulatedScore - score);
};

// 게임 점수 추가 및 목표 달성 여부 반환 (하위 호환성을 위해 유지, deprecated)
// @deprecated - 대신 addScoreToMemory()와 commitScore()를 사용하세요
export const addScore = (score: number): boolean => {
  const data = getDailyGoalData();
...
// 게임 점수 제거 (중도 포기 시) (deprecated)
// @deprecated - 대신 revertScore()를 사용하세요
export const removeScore = (score: number): void => {
  const data = getDailyGoalData();
  
  // 누적 점수에서 제거 (0 이하로 떨어지지 않도록)
  data.accumulatedScore = Math.max(0, data.accumulatedScore - score);
  
  // 점수가 목표보다 낮아지면 달성 취소
  if (data.targetScore !== null && data.accumulatedScore < data.targetScore) {
    data.achieved = false;
  }
